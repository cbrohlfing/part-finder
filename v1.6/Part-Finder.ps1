...
# Version: v1.6 (Per-folder enable/disable via checkboxes)
#
# Search engine:
    ...
    try {
        if (Test-Path -LiteralPath $path) {
            ...
            return [PSCustomObject]@{
                folders           = if ($cfg.folders) {
                    @(
                        $cfg.folders | ForEach-Object {
                            if ($_ -is [string]) {
                                [PSCustomObject]@{ path = ([string]$_); enabled = $true }
                            } elseif ($null -ne $_.path) {
                                $en = $true
                                try { if ($null -ne $_.enabled) { $en = [bool]$_.enabled } } catch {}
                                [PSCustomObject]@{ path = ([string]$_.path); enabled = $en }
                            } else {
                                [PSCustomObject]@{ path = ([string]$_); enabled = $true }
                            }
                        }
...
# Left container: Folders (top, grows/shrinks) + Metadata (bottom, fixed height)
# -------------------------
...
$listFolders.SelectionMode = 'MultiExtended'
$listFolders.IntegralHeight = $false

# Bottom controls panel (uses a TableLayoutPanel so labels can wrap and buttons can wrap)
...
$tblFolderBottom.Padding = New-Object System.Windows.Forms.Padding(0)
...
$lblAdd.MaximumSize = New-Object System.Drawing.Size(1000, 0)
...
$flowFolderBtns.WrapContents = $true
$flowFolderBtns.AutoSize = $true
$flowFolderBtns.AutoSizeMode = 'GrowAndShrink'
$flowFolderBtns.Dock = 'Top'
$flowFolderBtns.Padding = New-Object System.Windows.Forms.Padding(0, 6, 0, 0)
$flowFolderBtns.Margin = New-Object System.Windows.Forms.Padding(0)
...
$lblFolderHint.MaximumSize = New-Object System.Drawing.Size(1000, 0)
$lblFolderHint.Padding = New-Object System.Windows.Forms.Padding(0, 6, 0, 0)
...
$tblFolders.Controls.Add($tblFolderBottom, 0, 1)
...
$grpMeta.Dock = 'Bottom'
$grpMeta.Height = 250
$grpMeta.MinimumSize = New-Object System.Drawing.Size(0, 250)
...
$lblMetaOwn = $metaOwnRow[0]; $valMetaOwn = $metaOwnRow[1]
...
$tblMeta.Controls.Add($lblMetaOwn, 0, 6); $tblMeta.Controls.Add($valMetaOwn, 1, 6)
...
# Separator line between Folders and Metadata (more defined split)
$pnlLeftSep = New-Object System.Windows.Forms.Panel
$pnlLeftSep.Dock = 'Bottom'
$pnlLeftSep.Height = 6
$pnlLeftSep.BorderStyle = 'Fixed3D'
...
$pnlLeft.Controls.Add($grpFolders)
$pnlLeft.Controls.Add($pnlLeftSep)
$pnlLeft.Controls.Add($grpMeta)
